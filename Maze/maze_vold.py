...
def analyze_policy(path, policy, env):
    for i in range(len(path)):
        state = path[i]
        ...
        if i > 0:
            if path[i] == path[i-1]:
                ...
                print("At " + str(previous_state) + "The proba to go to " + str(state))
                print(env.transition_probabilities[env.map[state], env.map[previous_state], int(policy[env.map[previous_state], i-1])])# Proba of ending up in next state given current pos